...
use crate::math::{BinaryOperation, UnaryOperation, Object, Expression, FunctionRepr};
...
impl Parser {
    ...
    fn parse_comma_expression(&mut self, closer: &Token, constants: &mut HashMap<String, Object>, functions: &mut HashMap<String, FunctionRepr>) -> Result<Vec<Expression>, String> {
    ...
    fn parse_expression(&mut self, min_precedence: u8, constants: &mut HashMap<String, Object>, functions: &mut HashMap<String, FunctionRepr>) -> Result<Expression, String> {
                        ...
                        match self.peek() {
                            Token::Identifier(ident) if ident.starts_with('_') => {
                                let norm_type = if ident == "_" {
                                    ...
                                    let res = self.parse_expression(0, constants, functions)?;
                ...
                Token::Comparison(..) => {
                    ...
                    if let Token::Comparison(c, param) = self.next() { // Will always succeed
                        let parsed_param = if let Some(p) = param {
                            ...
                            Some(Box::new(Parser{tokens: p, pos: 0}.parse(constants, functions)?.into_iter().next().unwrap()))
                        } else {None};
    ...
    /// </table>
    pub fn parse(&mut self, constants: &mut HashMap<String, Object>, functions: &mut HashMap<String, FunctionRepr>) -> Result<Vec<Expression>, String> {
        let mut exprs = Vec::<Expression>::new();
        loop {
